...
if __name__ == "__main__":
    ...
    if st.button("Find Duplicates"):
        ...
        if results and 'df_exact' in results:
            ...
            # Add "Delete All Exact Duplicates" button
            if st.button("Delete All Exact Duplicates"):
                exact_duplicate_paths = list(set(df_exact_filtered['File1']) | set(df_exact_filtered['File2']))
                delete_duplicates(exact_duplicate_paths)
        ...
        if results and 'df_fuzzy' in results:
            ...
            # Add "Delete All Fuzzy Duplicates" button
            if st.button("Delete All Fuzzy Duplicates"):
                fuzzy_duplicate_paths = list(set(df_fuzzy_filtered['File1']) | set(df_fuzzy_filtered['File2']))
                delete_duplicates(fuzzy_duplicate_paths)

        st.write("Metadata Information:")
        if results and 'metadata_df' in results:
            st.dataframe(results['metadata_df'])

        # Add "Delete All Duplicates" button outside the if statement
    if results and deletion_status:
        all_duplicate_paths = list(set(df_exact_filtered['File1']) | set(df_exact_filtered['File2']) |
                                   set(df_fuzzy_filtered['File1']) | set(df_fuzzy_filtered['File2']))
        if st.button("Delete All Duplicates"):
            delete_duplicates(all_duplicate_paths)
            deletion_status = False  # Reset deletion status

            # Display updated results after deletion
            st.write("Exactly Same Files (Content Match = True):")
            if results and 'df_exact' in results:
                df_exact_filtered = results['df_exact'][results['df_exact']['ContentMatch'] == True]
                st.dataframe(df_exact_filtered)
