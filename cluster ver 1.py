...
for state, coordinates in state_coordinates.items():
    ...
    for i, feature in enumerate(geojson_data['features']):
        if feature['properties']['state'] == state and i < len(clusters):  # index within bounds
            ...
            folium.Marker([lat, long], popup=f'Latitude: {lat}, Longitude: {long}', icon=folium.Icon(color=f'lightblue')).add_to(marker_cluster)



map_clusters.save('cluster_map ver 1.html')
